...
export class Codec2<T extends { [K in keyof T]: K extends string ? T[K] : never }> {
...
export const Apply2 = <T extends any>(subschema: T, ...codecAddFns: CodecAddFn[]): T => {
...
export const Encode2 = (schema: any, inputData: any, ...codecs: Codec2<any>[]): any => {
...
export const Decode2 = (schema: any, inputData: any, ...codecs: Codec2<any>[]): any => {
  const sr = new SchemaReader(schema);
  return sr.map(inputData, (value, ...subschemas: any[]) => {
    let mapped: any = value;
    for (const codec of codecs) {
